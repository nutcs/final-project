...
def check_outNut():
    ...
    padX_right = 200
    #infomation 
    Label(mainFrame_checkout,text='Thaks Annซ๋า').grid(row=1,columnspan=2,sticky=SW,padx=padX_left,pady=10)
    Label(mainFrame_checkout,text='Your booking in kiev is confirmed',font="Calibri 20 bold").grid(row=2,columnspan=2,sticky=W,padx=padX_left)
    Label(mainFrame_checkout,text="Gar'is Kyiv Factory Hostel",font="Calibri 20 bold",fg='blue').grid(row=3,columnspan=2,sticky=W,padx=padX_left)
    #img
    Label(mainFrame_checkout,image=img_test).grid(row=4,columnspan=2,pady=20,sticky=W,padx=padX_left)

    #detel
    Label(mainFrame_checkout,text='Your reservation').grid(row=5,columnspan=2,sticky=W,padx=padX_left)
    Label(mainFrame_checkout,text=f'{1} night, {1} dormitory bed').grid(row=5,column=1,sticky=E,padx=padX_right)
    
    Label(mainFrame_checkout,text='check-in').grid(row=6,columnspan=2,sticky=W,padx=padX_left)
    Label(mainFrame_checkout,text=f'13/5/23').grid(row=6,columnspan=2,sticky=E,padx=padX_right)
    
    Label(mainFrame_checkout,text='Check out').grid(row=7,columnspan=2,sticky=W,padx=padX_left)
    Label(mainFrame_checkout,text='14/5/23').grid(row=7,columnspan=2,sticky=E,padx=padX_right)
    ...
    Button(mainFrame_checkout,text='Cancel your booking',bg='orange',fg='white').grid(row=9,columnspan=2,pady=20,sticky=S)
